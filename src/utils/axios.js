...
class API_INSTANCE {
 constructor ({ timeout = 1000000, url, token } = {}) {
 ...
 create(instance) {
  ...
  API.defaults.headers.common['Authorization'] = `Bearer ${this.token || Storage.get(config.authProps[0])}`;
  API.defaults.headers.common['ngrok-skip-browser-warning'] = true;

  const handleSessionExpired = (error) => {
   const msg = error?.response?.data?.message,
   message = msg instanceof Array ? msg[0] : msg;
   const isNotAuth = ['authorization', 'auth', 'authorized', 'access forbidden', 'jwt expired', 'jwt', 'Forbidden...You are using an expired token', 'invalid signature'].find(msg => error?.response?.data?.error?.toLowerCase().includes(msg?.toLowerCase()) || message?.toLowerCase().includes(msg?.toLowerCase()))
